...
if(deployToWeb) {
    ...
    app.get('/videvision/', function(req, res) {
        conn.query("SELECT Id, latitude__c, longitude__c FROM VideVision", function(err, result) {
            if (err) { res.json(err); }
...
function displayContactsSOQL() {
    conn.query("SELECT Id, latitude__c, longitude__c FROM VideVision__c", function(err, result) {
        if (err) { return console.error(err); }
        console.log("total : " + result.totalSize);
        for (var i=0; i<result.records.length; i++) {
            var record = result.records[i];
            console.log("Latitude: " + record.latitude__c);
            console.log("Longitude: " + record.longitude__c);
        }
      });
}
...
function displayContactsEventMethod() {
    ...
    var records = [];
    var query = conn.query("SELECT Id, latitude__c FROM VideVision__c")
...
function displayContactsMethodChain() {
    ...
    //
    console.log('method');
    conn.sobject("VideVision__c")
        .find({
        FirstName : { $like : 'Demo%' }
        },
        'Id, latitude__c, longitude__c' // fields can be string of comma-separated field names
                                // or array of field names (e.g. [ 'Id', 'Name', 'CreatedDate' ])
        )
        .sort('-latitude__c longitude__c') // if "-" is prefixed to field name, considered as descending.
        .limit(5)
...
function createContact() {
    console.log('create');
    conn.sobject("VideVision__c").create({latitude__c: '38.645699', longitude__c: '-90.202752'}, function(err, ret) {
...
function updateContact() {
    ...
    conn.query("SELECT Id, latitude__c FROM VideVision__c WHERE latitude__c = 'APIDemo'")
    .on("record", function(record) {
        conn.sobject("VideVision").update({Id: record.Id, LastName: 'Smith'}, function(err, ret) {
            if (err || !ret.success) { return console.error(err, ret); }
...
function deleteContact() {
    conn.query("SELECT Id, latitude__c FROM VideVision__c WHERE latitude__c = 'APIDemo'")
    .on("record", function(record) {
        conn.sobject("VideVision").delete(record.Id, function(err, ret) {
            if (err || !ret.success) { return console.error(err, ret); }
